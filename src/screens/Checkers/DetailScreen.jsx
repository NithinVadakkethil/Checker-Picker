...
//   const [recieptBatch, setRecieptBatch] = useState("")
...
//   const verifyReciept = async (purchaseId) => {
//     try {
//       setLoadingForMoveId(purchaseId, true);
//       const result = await checkerReceiptVerify(purchaseId)
//       toast.hideAll();
//       if (result?.success) {
//         setOrderStaus(true);
//         setRecieptBatch(result?.batchNo)
//         // setGroupedProducts((prevGroups) =>
//         //   prevGroups.map((group) =>
//         //     group.map((product) =>
//         //       product.move_id === moveId ? { ...product, state: "done" } : product
...
//                 }
//                 console.log("shouldShowOrderNo--->", shouldShowOrderNo)
//                 return (
//                   <ProductDetails
//                     key={product.move_id}
//                     moveId={product.move_id}
...
//                     expiryDate={product.lot_name || recieptBatch}
...
//                     verifyReciept={verifyReciept}
//                     purchaseId={product.purchase_id}
//                     recieptBatch={recieptBatch}
//                   />
//                 );
//               })}
//             </View>
...
const DetailScreen = ({ activeName, productLines }) => {
  ...
  const [groupedProducts, setGroupedProducts] = useState(() => {
    return Object.values(
      productLines?.reduce((acc, product) => {
        // Group by order_no instead of location
        const key = product.order_no;
        if (!acc[key]) {
          acc[key] = [];
        }
        acc[key].push(product);
  ...
  const verifyReciept = async (purchaseId) => {
    try {
      ...
      setLoadingForMoveId(purchaseId, false);
    }
  };
